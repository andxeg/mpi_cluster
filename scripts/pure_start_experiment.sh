...
RESULT_DIR="/home/mpiuser/cloud/experiments/results/pure_NPB_local" # dir in master node for results
MON_RES_DIR="/home/arccn/mpi/experiments/results/pure_NPB_local/monitor" # dir in host machine for monitoring results
    ...
    do
        ...
        do
            ...
            do
                ...
                printf "[Experiment #$k] RUN PROGRAM: ($PROG); NP: ($np); NODES: (%s); MONITOR: (%s) \n" "$cluster_nodes" "$monitor_nodes"

                # Start monitoring
                pidfile "/tmp/kvmtop.pid" sudo kvmtop -c qemu:///system --printer=text --cpu --net | pidfile "/tmp/grep.pid" grep -w "$monitor_nodes" &> $MON_RES_DIR/$PROG"_"$size"_"$np"_"mon_e_"$k".out &

                # Run MPI programs
                sshpass -p "mpiuser" ssh -o StrictHostKeyChecking=no -tT mpiuser@$MASTER "mpirun -np $np --hosts $cluster_nodes $PROG_DIR/$PROG &> $RESULT_DIR/$PROG\_$size\_$np\_e\_$k.out"
