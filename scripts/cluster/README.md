...
Также нужно убедиться, что все сервера, на которых будет разворачиваться кластер доступны друг другу и можно зайти по ssh.
Если возникнут какие-то проблемы с сетью, то см. Примечания.

# Решение
Пусть есть несколько серверов: `server1`, ..., `serverM`, и стоит задача сделать MPI кластер.

Сначала нужно сформировать конфигурационный файл, в котором описать все сервера, их параметры (CPU, RAM, HDD, IP address, user, password). Также нужно указать сервер, на котором будет располагаться программный коммутатор, через который будут проходить TCP потоки между узлами кластера. Шаблон конфигурационного файла -> `./shared/config.template`. В конфигурационном файле можно указать количество виртуальных машин, которые нужно развернуть на конкретном сервере. Если это количество не указано, то будет развернуто максимальное количество виртуальных машин в зависимости от параметров `vCPU` и `RAM`, которые указываются при запуске скрипта `shared_cluster.py`.

Чтобы создать виртуальный кластер нужно выполнить команду
```bash
...
Где `first` - это имя кластера, `test_config.json` - конфигурационный файл с описанием серверов, на которых будет развернут кластер,
`start_vms.sh` - скрипт для поднятия виртуальных машин на одном сервере, `configure_vms.sh` - скрипт для настройки виртуальных машин во всем кластере, `node_setup.sh` - скрипт для настройки одной виртуальной машины, `linux_bridge_up.sh` - скрипт для настройки программного коммутатора, через который проходит весь трафик виртуальных машин в кластере, `cpu` - количество ядер у одной виртуальной машины, `ram` - количество RAM у одной виртуальной машины.

Чтобы удалить виртуальный кластер нужно выполнить команду
```bash
python shared_cluster.py delete -na  first -co  test_config.json -cds ../remote/clear.sh -bds ../remote/linux_bridge_down.sh
```

Где `first` - это имя кластера, `test_config.json` - конфигурационный файл с описанием серверов, на которых будет развернут кластер,
`clear.sh` - скрипт для удаления части кластера в рамках одного сервера, `linux_bridge_down.sh` - скрипт для удаления программного
коммутатора через который проходит весь трафик виртуальных машин в кластере.
